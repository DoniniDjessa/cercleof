-- Create dd-products table for beauty institute product management
CREATE TABLE IF NOT EXISTS "dd-products" (
  ...
  barcode VARCHAR(100),
  category_id UUID REFERENCES "dd-product-categories"(id),
  brand VARCHAR(100),
...
-- Create dd-product-categories table
CREATE TABLE IF NOT EXISTS "dd-product-categories" (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name VARCHAR(100) NOT NULL,
  description TEXT,
  parent_id UUID REFERENCES "dd-product-categories"(id),
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_by UUID REFERENCES "dd-users"(id)
...
-- Create dd-services table for beauty institute service management
CREATE TABLE IF NOT EXISTS "dd-services" (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name VARCHAR(255) NOT NULL,
  description TEXT,
  category_id UUID REFERENCES "dd-service-categories"(id),
...
-- Create dd-service-categories table
CREATE TABLE IF NOT EXISTS "dd-service-categories" (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  name VARCHAR(100) NOT NULL,
  description TEXT,
  parent_id UUID REFERENCES "dd-service-categories"(id),
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  created_by UUID REFERENCES "dd-users"(id)
...
ALTER TABLE "dd-product-categories" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "dd-service-categories" ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "All users can view product categories" ON "dd-product-categories"
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "All users can create product categories" ON "dd-product-categories"
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "All users can update product categories" ON "dd-product-categories"
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Admin and manager can delete product categories" ON "dd-product-categories"
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM "dd-users" 
            WHERE auth_user_id = auth.uid() 
...
CREATE POLICY "All users can view service categories" ON "dd-service-categories"
    FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "All users can create service categories" ON "dd-service-categories"
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "All users can update service categories" ON "dd-service-categories"
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Admin and manager can delete service categories" ON "dd-service-categories"
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM "dd-users" 
            WHERE auth_user_id = auth.uid() 
...
GRANT ALL ON "dd-product-categories" TO authenticated;
GRANT ALL ON "dd-service-categories" TO authenticated;
