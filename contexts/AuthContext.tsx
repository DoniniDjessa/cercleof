...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const getInitialSession = async () => {
      try {
        // TEMPORARY: Block session restoration - Remove these lines to re-enable
        setSession(null);
        setUser(null);
        setLoading(false);
        return;
          ...
          try {
            ...
            const timeoutPromise = new Promise<{
              ...
              (resolve) =>
                setTimeout(() => {
                  console.log(
                    "AuthContext: User check timeout (5s), continuing with session but will re-check in background",
                  );
            ...
            if (!userResult.exists) {
              console.log(
                "AuthContext: User not found in dd-users, disconnecting...",
              );
            ...
            console.error(
              "AuthContext: Error checking user existence:",
              checkError,
            );
            // On error, don't disconnect immediately - might be a network issue
            // Set session anyway and let the timeout handler re-check
            console.log(
              "AuthContext: Error during check, but continuing with session (might be network issue)",
            );
            setSession(session);
    ...
    timeoutId = setTimeout(async () => {
      console.log(
        "AuthContext: Timeout reached (10s), checking session and user status...",
      );
      ...
      try {
        ...
        if (!currentSession || !currentSession.user) {
          console.log(
            "AuthContext: No session after timeout, disconnecting immediately...",
          );
          // Clear state immediately
          setSession(null);
          setUser(null);
          ...
          try {
            ...
            const timeoutPromise = new Promise<{
              ...
              (resolve) =>
                setTimeout(() => {
                  console.log(
                    "AuthContext: User check timeout in timeout handler, but continuing with session",
                  );
            ...
            if (!userResult.exists) {
              console.log(
                "AuthContext: User not found in dd-users after timeout - disconnecting immediately...",
              );
              // Clear state immediately
              setSession(null);
              setUser(null);
            ...
            console.error(
              "AuthContext: Error checking user after timeout:",
              checkError,
            );
            // If check fails, don't disconnect - might be a network issue
            // Just stop loading and continue with session
            console.log(
              "AuthContext: Error checking user after timeout, but continuing with session",
            );
            setLoading(false);
            return;
          }
        ...
        console.log(
          "AuthContext: Valid session and verified user found after timeout, stopping loading...",
        );
        setLoading(false);
      } catch (timeoutError) {
        console.error(
          "AuthContext: Error checking session after timeout:",
          timeoutError,
        );
        // On error, don't disconnect - might be a network issue
        // Just stop loading and continue with existing session if any
        console.log(
          "AuthContext: Error checking session after timeout, but continuing with existing session",
        );
        setLoading(false);
      }
        ...
        try {
          ...
          const timeoutPromise = new Promise<{
            ...
            (resolve) =>
              setTimeout(() => {
                console.log(
                  "AuthContext: User check timeout in onAuthStateChange, but continuing with session",
                );
                resolve({ exists: true }); // Continue with session even if timeout - might be network issue
          ...
          if (!userResult.exists) {
            console.log(
              "AuthContext: User not found in dd-users (SIGNED_IN), disconnecting...",
            );
            // Clear state immediately
            setSession(null);
            setUser(null);
          ...
          // User exists - set session and user with role
          console.log(
            "AuthContext: User verified in onAuthStateChange, setting session and user",
          );
          setSession(session);
          const userWithRole = {
            ...session.user,
          ...
          console.error(
            "AuthContext: Error checking user in onAuthStateChange:",
            checkError,
          );
          // On error, don't disconnect - might be a network issue
          // Set session anyway and continue
          console.log(
            "AuthContext: Error during check in onAuthStateChange, but continuing with session",
          );
          setSession(session);
          setUser(session?.user ?? null);
          setLoading(false);
  ...
  const checkUserExists = async (
    ...
    try {
      ...
      console.log(
        "AuthContext: User found in dd-users:",
        data?.pseudo,
        "with role:",
        data?.role,
      );
  ...
  const signUp = async (email: string, password: string) => {
    ...
    if (result.data?.user && !result.error) {
      try {
        ...
        if (linkError) {
          console.log(
            "No existing profile found to link, user will need to complete setup",
          );
        } else {
